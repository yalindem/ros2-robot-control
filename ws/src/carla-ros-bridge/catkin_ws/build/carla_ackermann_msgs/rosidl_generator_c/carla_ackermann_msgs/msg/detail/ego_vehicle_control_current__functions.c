  ...
  carla_ackermann_msgs__msg__EgoVehicleControlCurrent__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    carla_ackermann_msgs__msg__EgoVehicleControlCurrent * data =
      (carla_ackermann_msgs__msg__EgoVehicleControlCurrent *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!carla_ackermann_msgs__msg__EgoVehicleControlCurrent__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          carla_ackermann_msgs__msg__EgoVehicleControlCurrent__fini(&output->data[i]);
        }
        return false;
      }
    }
