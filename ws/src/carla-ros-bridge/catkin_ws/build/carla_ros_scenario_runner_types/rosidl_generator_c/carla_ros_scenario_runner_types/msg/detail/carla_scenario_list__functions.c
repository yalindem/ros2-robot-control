  ...
  carla_ros_scenario_runner_types__msg__CarlaScenarioList__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    carla_ros_scenario_runner_types__msg__CarlaScenarioList * data =
      (carla_ros_scenario_runner_types__msg__CarlaScenarioList *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!carla_ros_scenario_runner_types__msg__CarlaScenarioList__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          carla_ros_scenario_runner_types__msg__CarlaScenarioList__fini(&output->data[i]);
        }
        return false;
      }
    }
