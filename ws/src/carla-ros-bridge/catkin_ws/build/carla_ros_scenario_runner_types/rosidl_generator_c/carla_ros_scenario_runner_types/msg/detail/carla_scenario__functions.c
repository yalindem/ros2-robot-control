  ...
  carla_ros_scenario_runner_types__msg__CarlaScenario__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    carla_ros_scenario_runner_types__msg__CarlaScenario * data =
      (carla_ros_scenario_runner_types__msg__CarlaScenario *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!carla_ros_scenario_runner_types__msg__CarlaScenario__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          carla_ros_scenario_runner_types__msg__CarlaScenario__fini(&output->data[i]);
        }
        return false;
      }
    }
