  ...
  carla_waypoint_types__msg__CarlaWaypoint__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    carla_waypoint_types__msg__CarlaWaypoint * data =
      (carla_waypoint_types__msg__CarlaWaypoint *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!carla_waypoint_types__msg__CarlaWaypoint__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          carla_waypoint_types__msg__CarlaWaypoint__fini(&output->data[i]);
        }
        return false;
      }
    }
